...
import csv
import os
import config as config  # Cambiado a importación absoluta
import Data_base as db    # Cambiado a importación absoluta
import helpers as helpers  # Cambiado a importación absoluta

class TestDatabase(unittest.TestCase):
    def setUp(self):
        # Configurar la ruta del archivo temporal para los tests
        self.test_db_path = os.path.join(os.path.dirname(__file__), "clientes_test.csv")
        config.DATABASE_PATH = self.test_db_path

        # Crear el archivo temporal con datos iniciales
        with open(self.test_db_path, "w", newline="\n") as fichero:
    ...
    def test_modificar_cliente(self):
        cliente_a_modificar = copy.deepcopy(self.clientes.buscar('28Z'))
    ...
    def test_escritura_csv(self):
        ...
        with open(self.test_db_path, newline="\n") as fichero:
    ...
    def test_dni_valido(self):
        ...
        self.assertTrue(helpers.validar_dni("00A"))  # Válido: 2 dígitos + 1 letra
        self.assertFalse(helpers.validar_dni("23223S"))  # Inválido: más de 3 caracteres
        self.assertFalse(helpers.validar_dni("F35"))  # Inválido: letra al inicio
        self.assertTrue(helpers.validar_dni("48H"))  # Válido: 2 dígitos + 1 letra
